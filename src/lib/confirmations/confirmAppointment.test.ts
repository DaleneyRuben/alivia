...
describe("confirmAppointment", () => {
  ...
  it("throws if the appointment belongs to a different doctor", async () => {
    ...
    await expect(confirmAppointment("a1")).rejects.toThrow("Not authorized");
    expect(update).not.toHaveBeenCalled();
  });

  it("marks a SCHEDULED appointment confirmed", async () => {
    findUnique.mockResolvedValue({
      id: "a1",
      doctorId: DOCTOR_ID,
      status: "SCHEDULED",
  ...
  it.each(["ATTENDED", "NO_SHOW", "CANCELLED"] as const)(
    "rejects confirming an appointment already in the final state %s",
    async (finalStatus) => {
      findUnique.mockResolvedValue({
        id: "a1",
        doctorId: DOCTOR_ID,
      ...
      await expect(confirmAppointment("a1")).rejects.toThrow(
